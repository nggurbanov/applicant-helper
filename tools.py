...
async def generate_dialog(message):
    messages = [{"role":"system", "content":NON_FOUND_PROMPT},
                {"role":"message_history", "content":context_to_text()},
...
async def reply(text, dialog_mode=False):
    number = int(await find_question(text))
    if number == 0:
        response = await generate_short(text) if not dialog_mode else await generate_dialog(text)
    else:
...
async def formatted_reply(text, dialog_mode=False):
    reply_msg = await reply(text, dialog_mode)
